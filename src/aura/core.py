    ...
    *,
    ...
    if _is_date_only(time_or_date):
        ...
        if not volumes:
            ...
            raise FileNotFoundError(f"No data found for radar {rid} on {time_or_date}. " f"Expected zip file: {zp}")
        return volumes
    ...
    if isinstance(time_or_date, datetime):
        ...
        if is_midnight and not nearest:
            ...
            if not volumes:
                zp = zip_path(rid, day.year, day.month, day.day)
                raise FileNotFoundError(f"No data found for radar {rid} on {day}. " f"Expected zip file: {zp}")
            return volumes
    ...
    raise TypeError(f"time_or_date must be a date or datetime, got {type(time_or_date).__name__}")
